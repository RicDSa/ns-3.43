...
HwmpProtocol::GetActivePeerLinks() const
{
    // Pull the PeerManagementProtocol out of the mesh‐point device
    Ptr<PeerManagementProtocol> pmp = m_device->GetObject<PeerManagementProtocol>();
    if (!pmp)
    {
        // either the device wasn’t set, or the PMP object
        // wasn’t aggregated onto it
        return {};
    }
...
{
    // --- Active peer links ---
    // Get the active peer links from the PeerManagementProtocol
    // and build HWMP path requests toward each peer.
    auto links = GetActivePeerLinks();
    for (auto link : links)
    {
    ...
    NS_LOG_DEBUG("Last active peer addresses:");
...
HwmpProtocol::DoLinkCheck()
{
    ...
    {
        current.insert(link->GetPeerAddress());
    }

    // compare with last run (optional):
    for (auto added : current)
    {
        if (m_activePeers.count(added) == 0)
        {
            NS_LOG_INFO("[" << Simulator::Now().GetSeconds() << "s] Link UP to " << added);
        }
    }
    for (auto old : m_activePeers)
    {
        if (current.count(old) == 0)
        {
            NS_LOG_INFO("[" << Simulator::Now().GetSeconds() << "s] Link DOWN from " << old);
        }
    }

    // swap into your storage
    m_activePeers.swap(current);
...
{
    NS_LOG_FUNCTION(this << src << dst << multicastGroup);
...
{
    NS_LOG_FUNCTION(this << (uint32_t)ttl);
    ...
    {
        double diff = ttl - m_nodeAvgTtl;
        NS_LOG_DEBUG("Node " << GetAddress() << " observed TTL = " << (uint32_t)ttl
                                               << ", diff = " << diff << ", old avgTTL = " << m_nodeAvgTtl);
    ...
    NS_LOG_INFO(
        "[HWMP] Node " << GetAddress() << " updated node-level average TTL = " << m_nodeAvgTtl
                            ...
                            uint16_t protocolType, RouteReplyCallback routeReply)
{
            ...
            if (ShouldPrune(constPacket, destination) || !hasOtherPeers) 
            {
                ...
                if (!amInterested || isDuplicate) 
                {
                    ...
                    if (m_lastPruneSent.IsZero() || Simulator::Now() - m_lastPruneSent > Seconds(3.0) || isDuplicate) 
                    {
                        ...
                        pruneInfo.push_back(std::make_pair(destination, 0)); 
    ...
    if (destination.IsGroup())
    {
        ...
        for (auto plugin = m_interfaces.begin(); plugin != m_interfaces.end(); plugin++)
        {
            ...
            if (receivers.empty())
            {
                NS_LOG_UNCOND("Receivers for interface " << plugin->first << " is empty.");
            }
            else
            {
                std::stringstream ss;
                ss << "Receivers for interface " << plugin->first << ": ";
            ...
            for (auto i = receivers.begin(); i != receivers.end(); i++)
            {
                ...
                for (const auto& entry : peerlinks)
                {
                    ...
                    if ((entry.second == 0 || entry.second == 2 ) && peer!=transmitter) // peer is considered bellow and beside him is was not the transmitter
                    {
                        ...
                        {
                            NS_LOG_DEBUG("Prune active for " << GetAddress() << " -> " << peer
                                                             << " for multicast group "
                                                             << destination
                                                             << ". Dropping packet.");
                            prunedCount++;
                        ...
                        {
                            unprunedPeers.push_back(peer);
                        }
                    }
                }

                NS_LOG_DEBUG("Pruned count: " << prunedCount
                                              << ", total peer links down: " << peerCount);

                NS_LOG_DEBUG("Peer link size: " << peerlinks.size());
                // list all peer links
                NS_LOG_DEBUG("Last active peer addresses:" << GetAddress());
                for (const auto& addr : m_lastActivePeerAddrs)
                {
                    NS_LOG_DEBUG("Address: " << addr);
                }
                ...
                if(m_enableFloodAndPrune){
                    ...
                    if (m_RxPacketCount <= 5)
                    {
                        NS_LOG_DEBUG("Skip prune for the two first packets");
                    }
                    else
                    {
                        if (prunedCount == peerCount)
                        {
                            if (GetAddress() == "00:00:00:00:00:01")
                            {
                                NS_LOG_DEBUG("Skip prune");
                            }
                            else
                            {
                             ...
                             uint32_t ttl)
{
    ...
    NS_ASSERT(destination.IsGroup() == false);
                         ...
                         bool use)
{
    ...
    // Impede que ecos de vizinhos ao mesmo nível corrompam quem é Pai/Filho
    uint64_t packetUid = packet->GetUid();
    for (auto it = m_seenPackets.begin(); it != m_seenPackets.end(); ++it) {
        if (*it == packetUid) {
            NS_LOG_DEBUG("StartPrune: Eco/Duplicado detectado (" << packetUid << "). Ignorando para não corromper a árvore.");
            return; 
        }
    }
    ...
    NS_LOG_DEBUG("Current prune table1:");
    for (const auto& entry : m_pruneTable)
    {
